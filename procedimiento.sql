...
BEGIN
    ...
    RETURN CAST(CAST(GETUTCDATE() AS DATETIMEOFFSET) AT TIME ZONE 'CENTRAL STANDAR
	D TIME (MEXICO)' AS DATETIME)
END
GO
...
BEGIN
	...
	BEGIN
		...
		UPDATE ARBOL SET ID_VALOR=@ID_VALOR
	END
END;
...
SET @ID_VALOR_INFO=(SELECT ID_VALOR FROM ARBOL WHERE ID_ARBOL=@ID_ARBOL);
IF @ID_VALOR_INFO IS NULL
BEGIN
	SELECT A.NOMBRE_CIENTIFICO,A.FECHA_PLANTADO,C.DESCRIPCION,C.FRECUENCIA FROM ARBOL A JOIN CUIDADOS C ON A.ID_ARBOL=C.ID_CUIDADOS
END
ELSE
BEGIN
	SELECT A.NOMBRE_CIENTIFICO,A.FECHA_PLANTADO,C.DESCRIPCION,C.FRECUENCIA,V.VALOR FROM ARBOL A JOIN CUIDADOS C ON A.ID_ARBOL=C.ID_CUIDADOS JOIN VALORES V ON A.ID_VALOR=V.ID_VALOR
END

--AGREGAR SERVICIO
GO
CREATE OR ALTER PROCEDURE AGREGAR_SERVICIO(@ID_ARBOL INT,@ID_SERVICIO INT,@COMENTARIOS VARCHAR(255),@ESTADO VARCHAR(50),@ALTURA FLOAT,@CIRCUNFERENCIA FLOAT,@ID_USUARIO INT)
AS
BEGIN
	IF @ID_SERVICIO=2
	BEGIN
		INSERT INTO BRINDAN(ID_ARBOL,ID_SERVICIO,FECHA_SERVICIO,COMENTARIOS,ESTADO) VALUES(@ID_ARBOL,@ID_SERVICIO,(SELECT DBO.OBTENER_FECHA()),@COMENTARIOS,@ESTADO);
...
WHERE 
    DATEDIFF(DAY, B.FECHA_SERVICIO, (SELECT DBO.OBTENER_FECHA())) >= C.FRECUENCIA;

--ENVIAR NOTIFICACIONES
GO
CREATE OR ALTER PROCEDURE EnviarRecordatorioActividad
...
BEGIN
    ...
    FROM 
        ADOPTA A
    WHERE 
        EXISTS (SELECT 1 FROM ADMINISTRADOR WHERE ID_US = @ID_Administrador);
END;
...
BEGIN
    ...
    INSERT INTO REPORTE (FECHA, PROGRESO, ID_ARBOL)
    ...
    SELECT 
        ...
        A.ALTURA AS Altura,
        A.CIRCUNFERENCIA AS Circunferencia,
    ...
    JOIN 
        Colonia C ON A.ID_COL = C.ID_COLONIA
    WHERE 
...
--AGREGAR ARBOLES
GO
CREATE OR ALTER PROCEDURE ALTA_ARBOLES(@NOMBRE_CIENTIFICO VARCHAR(50),@FECHA_PLANTADO DATE,@ENDEMICO BIT,@ID_CUIDADOS INT,@ID_COL INT)
AS
BEGIN
	INSERT INTO ARBOL(NOMBRE_CIENTIFICO,FECHA_PLANTADO,ENDEMICO,ID_VALOR,ID_CUIDADOS,ID_COL) VALUES(@NOMBRE_CIENTIFICO,@FECHA_PLANTADO,@ENDEMICO,NULL,@ID_CUIDADOS,@ID_COL)
END;
